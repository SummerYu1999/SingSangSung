    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes lively-fountain {
            ...
            15% { opacity: 1; transform: translate(calc(var(--mx) * 0.5), -60px) scale(1.4) rotate(var(--r1)); }
            100% { transform: translate(var(--dx), var(--dy)) scale(0.4) rotate(var(--r2)); opacity: 0; }
        }
...
<body onclick="document.getElementById('invisible-input').focus()">
    ...
    function loadSong() {
        ...
        // 檢查外部資料是否存在
        if (typeof songData !== 'undefined' && songData[key]) {
    ...
    document.getElementById('invisible-input').addEventListener('input', (e) => {
        ...
        if (bv) {
            ...
            return; // 觸發了合聲就不噴一般 Emoji
        }

        // 2. 噴發一般 Emoji (全部從當前歌曲資料 current 中抓取)
        if (e.data && current) {
            // 先看這首歌有沒有針對這個單字的專屬 Emoji Map
            const specialList = (current.emojiMap && current.emojiMap[lastWord]) ? current.emojiMap[lastWord] : null;
            // 如果沒有專屬單字特效，就用這首歌的通用 Emoji 庫 (customEmojis)
            const list = specialList || current.customEmojis || [];
