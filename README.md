...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        #scroll-engine { transition: transform 0.4s cubic-bezier(0.23, 1, 0.32, 1); will-change: transform; transform: translateY(0); }
        ...
        /* 光標 */
        .cursor { border-left: 3px solid var(--primary); animation: blink 0.8s infinite; display: inline-block; height: 1.1em; vertical-align: middle; margin-left: 1px; box-shadow: 0 0 10px var(--primary); }
        @keyframes blink { 50% { opacity: 0; } }

        /* 完全隱藏輸入框但保持功能 */
        #invisible-input { position: absolute; opacity: 0; pointer-events: none; left: -9999px; }

        .sidebar { display: flex; flex-direction: column; gap: 15px; }
        .card { background: var(--card); padding: 20px; border-radius: 14px; border-left: 5px solid var(--primary); box-shadow: 0 8px 20px rgba(0,0,0,0.4); transition: 0.3s; }
...
<body onclick="document.getElementById('invisible-input').focus()">
    ...
    function loadSong() {
        ...
        // 渲染並強制滾回頂部
        render();
        document.getElementById('scroll-engine').style.transform = `translateY(0px)`;
        input.focus();
    }
    ...
    function render() {
        ...
        for (let i = 0; i < target.length; i++) {
            ...
            // 修正光標插入邏輯：光標應該是在下一個要打的位置
            if (i === val.length) html += `<span class="cursor"></span>`;
        ...
        // 如果打完了，補一個光標在最後
        if (val.length >= target.length) html += `<span class="cursor"></span>`;
        ...
        if (val.length === 0) {
            ...
            scrollEngine.style.transform = `translateY(-${offset - 80}px)`;
        }
    }
    ...
    function updateStats() {
        ...
        if (targetVocab) {
            ...
            document.getElementById('vocab-hint').innerHTML = `
                ...
                <div style="color:#aaa; font-size:14px;">${targetVocab.k}</div>
